...
aux_taxa <- 1:J
names(aux_taxa) <- colnames(X)

#get the metadata and taxonomy of the selected rows and samples
X_meta_c <- X_meta[aux_samples,]
X_taxo_c <- X_taxo[aux_taxa,]

#visualize the distribution of the selection
...
X_meta_c_diag <- X_meta_c[X_meta_c$diagnosis=='IBD_CD',]
aux_c_diag <- aux_c[rownames(X_meta_c_diag),]

#select by dysbiotic status and get a submatrix from the bicluster
X_meta_c_dysb <- X_meta_c[X_meta_c$dysbiotic=='T',]
aux_c_dysb <- aux_c[rownames(X_meta_c_dysb),]

#visualize the selections to find patterns
heatmap(aux_c_diag, Rowv=NA, Colv=NA, scale="none", main=paste('Cluster',cc,'(IBD_CD)'),cexRow=1,cexCol=0.25)

heatmap(aux_c_dysb, Rowv=NA, Colv=NA, scale="none", main=paste('Cluster',cc,'(dysbiotic)'),cexRow=1,cexCol=0.25)

#get the median abundance of taxa by diagnosis and by dysbiotic status
aux_c_diag_colsum_median <- apply(aux_c_diag,2,median)
print('Top 10 median abundance taxa in CD:')
head(sort(aux_c_diag_colsum_median[aux_c_diag_colsum_median>0], decreasing = TRUE),n=10)
print('----------------------------------------')
aux_c_dysb_colsum_median <- apply(aux_c_dysb,2,median)
print('Top 10 median abundance taxa in DYSBIOTIC:')
head(sort(aux_c_dysb_colsum_median[aux_c_dysb_colsum_median>0], decreasing = TRUE),n=10)
```
